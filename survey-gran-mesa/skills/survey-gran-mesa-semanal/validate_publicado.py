...
def find_html() -> Path:
    ...
    rel_dir = data_dir / 'gran-mesa' / 'relatorios'
    if not rel_dir.exists():
        raise FileNotFoundError(f"Pasta {rel_dir} não encontrada.")
    candidates = sorted(rel_dir.glob('Survey_Gran_Mesa_*_v2.html'),
                        key=lambda p: p.stat().st_mtime, reverse=True)
    if not candidates:
        raise FileNotFoundError("Nenhum HTML Survey_Gran_Mesa_*_v2.html encontrado.")
    return candidates[0]
